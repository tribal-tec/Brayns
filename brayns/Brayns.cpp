...
struct Brayns::Impl : public PluginAPI
{
    ...
    void createEngine()
    {
        ...
        void buildScene()
        {
            ...
            const auto frameSize =
                Vector2f(_engine->getFrameBuffer().getSize());
    ...
    private:
        ...
        void _updateAnimation()
        {
            ...
            {
                animParams.setFrame(animParams.getFrame() +
                                    animParams.getDelta());
            }
        }
...
#pragma omp critical
            ...
            if (!colorMapFilename.empty())
            {
                ...
                transferFunctionLoader.loadFromFile(
                    colorMapFilename, sceneParameters.getColorMapRange(),
                    scene);
            }
        ...
        */
        void _loadPDBFile(const std::string& filename)
        {
            ...
            if (!proteinLoader.importPDBFile(pdbFile, Vector3f(0, 0, 0), 0,
                                             scene))
        ...
        */
        void _loadXYZBFile(const Loader::UpdateCallback& progressUpdate)
        {
            ...
            try
            {
                xyzbLoader.importFromFile(geometryParameters.getXYZBFile(),
                                          scene, Matrix4f(), NO_MATERIAL);
            }
            catch (const std::runtime_error& e)
            {
                BRAYNS_ERROR << "Failed to import "
                             << geometryParameters.getXYZBFile() << ": "
                             << e.what() << std::endl;
            }
        }
        ...
        */
                             ...
                             const Loader::UpdateCallback& progressUpdate)
        {
            ...
            for (const auto& file : files)
            {
                size_t material = geometryParameters.getColorScheme() ==
                                          ColorScheme::neuron_by_id
                                      ? NB_SYSTEM_MATERIALS + i
                                      : NO_MATERIAL;

                try
                {
                    _meshLoader.importFromFile(file, scene, Matrix4f(),
                                               material);
                }
                catch (...)
                {
        ...
        void _loadNESTCircuit()
        {
            ...
            if (!circuit.empty())
            {
                ...
                const std::string& cacheFile(
                    geometryParameters.getNESTCacheFile());
        ...
        */
            ...
            const Loader::UpdateCallback& progressUpdate)
        {
            ...
            const auto& targets =
                geometryParameters.getCircuitTargetsAsStrings();
        ...
        */
        ...
        void _registerKeyboardShortcuts()
        {
            ...
            _keyboardHandler.registerKeyboardShortcut(
                ...
                std::bind(&Brayns::Impl::_decreaseAmbientOcclusionStrength,
                          this));
            _keyboardHandler.registerKeyboardShortcut(
                'O', "Increase ambient occlusion strength",
                std::bind(&Brayns::Impl::_increaseAmbientOcclusionStrength,
                          this));
            _keyboardHandler.registerKeyboardShortcut(
        ...
        void _toggleShadows()
        {
            RenderingParameters& renderParams =
                _parametersManager.getRenderingParameters();
            renderParams.setShadows(renderParams.getShadows() == 0.f ? 1.f
                                                                     : 0.f);
        }
        ...
        void _toggleSoftShadows()
        {
            RenderingParameters& renderParams =
                _parametersManager.getRenderingParameters();
            renderParams.setSoftShadows(
                renderParams.getSoftShadows() == 0.f ? 0.1f : 0.f);
        }
