...
enum class task_type
{
    ...
    accept_any,  // The task's functor accepts the first parent future that
                 // becomes ready
    consume,     // The task's functor consumes all parent results
    consume_any, // The task's functor consumes the first parent result that
                 // becomes ready
    wait, // The task's functor takes no arguments but waits for all parents to
          // finish
    wait_any, // The task's functor takes no arguments but waits for the first
              // parent to finish
};
...
struct root_type
    : std::integral_constant<transwarp::task_type, transwarp::task_type::root>
{
};
...
struct accept_type
    : std::integral_constant<transwarp::task_type, transwarp::task_type::accept>
{
};
...
struct accept_any_type
    : std::integral_constant<transwarp::task_type,
...
struct consume_type : std::integral_constant<transwarp::task_type,
...
struct wait_type
    : std::integral_constant<transwarp::task_type, transwarp::task_type::wait>
{
};
constexpr transwarp::wait_type wait{};

// The wait_any type. Used for tag dispatch
struct wait_any_type : std::integral_constant<transwarp::task_type,
...
public:
    ...
    // The task type
    transwarp::task_type get_type() const noexcept { return type_; }
    ...
    std::size_t get_priority() const noexcept { return priority_; }
    ...
    bool is_canceled() const noexcept { return canceled_.load(); }
private:
...
{
    return transwarp::to_string(*edge.get_parent(), separator) + " -> " +
           transwarp::to_string(*edge.get_child(), separator);
}
...
public:
    ...
    // Is supposed to run a task which is wrapped by the functor. The functor
    // only
    // captures a shared_ptr and can hence be copied at low cost. node
    // represents
    // the task that the functor belongs to.
    virtual void execute(const std::function<void()>& functor,
                         const std::shared_ptr<transwarp::node>& node) = 0;
};
...
public:
    ...
    virtual void set_executor(
        std::shared_ptr<transwarp::executor> executor) = 0;
    virtual void set_executor_all(
        std::shared_ptr<transwarp::executor> executor) = 0;
    ...
    virtual const std::shared_ptr<transwarp::node>& get_node() const
        noexcept = 0;
    ...
    virtual void schedule_all(transwarp::executor& executor,
                              bool reset_all = true) = 0;
...
private:
    ...
    virtual void schedule_impl(bool reset,
                               transwarp::executor* executor = nullptr) = 0;
};
...
{
    using type =
        typename rinfo_impl<ResultType, std::is_void<ResultType>::value>::type;
};
...
public:
    ...
    virtual const std::shared_future<ResultType>& get_future() const
        noexcept = 0;
...
// A base class for a user-defined functor that needs access to the node
// associated
...
protected:
    ...
    // If the associated task is canceled then this will throw
    ...
    void transwarp_cancel_point() const
    {
        ...
        {
            throw transwarp::task_canceled(
                std::to_string(transwarp_node_->get_id()));
        }
    }
...
struct node_manip
{
    static void set_type(transwarp::node& node,
                         transwarp::task_type type) noexcept
    {
        node.type_ = type;
    }

    static void set_name(transwarp::node& node,
    ...
    static void add_parent(transwarp::node& node,
                           std::shared_ptr<transwarp::node> parent)
    {
        node.parents_.push_back(std::move(parent));
    }

    static void set_executor(transwarp::node& node,
                             std::shared_ptr<std::string> executor) noexcept
    {
        if (executor)
        {
    ...
    static void set_priority(transwarp::node& node,
                             std::size_t priority) noexcept
    {
        node.priority_ = priority;
    }

    static void set_custom_data(transwarp::node& node,
                                std::shared_ptr<void> custom_data)
    {
        if (custom_data)
        {
...
public:
        ...
        : done_(false)
    {
        if (n_threads == 0)
        {
            throw transwarp::thread_pool_error(
                "number of threads must be larger than zero");
        }
    ...
    void worker()
    {
        for (;;)
        {
            std::function<void()> functor;
            {
                ...
                cond_var_.wait(lock,
                               [this] { return done_ || !functors_.empty(); });
...
FutureResult wait_for_any(const Futures&... futures)
{
    ...
    {
        auto future =
            transwarp::detail::wait_for_any_impl<FutureResult>(futures...);
        if (future.valid())
        {
            return future;
        }
...
struct call_with_futures_impl
{
    ...
    static Result work(std::size_t node_id, const Task& task,
                       const Tuple& futures)
    {
        return call_with_futures_impl<
            TaskType, total == 1 + static_cast<int>(sizeof...(n)), total, n...,
            static_cast<int>(sizeof...(n))>::template work<Result>(node_id,
                                                                   task,
                                                                   futures);
    }
};
...
struct call_with_futures_impl<transwarp::accept_any_type, true, total, n...>
{
    ...
    static Result work(std::size_t node_id, const Task& task,
                       const Tuple& futures)
    {
        using future_t = typename std::remove_reference<decltype(
            std::get<0>(futures))>::type; // use first type as reference
        auto future =
            transwarp::detail::wait_for_any<future_t>(std::get<n>(futures)...);
...
struct call_with_futures_impl<transwarp::consume_type, true, total, n...>
{
    ...
    {
        transwarp::detail::wait_for_all(std::get<n>(futures)...);
        return transwarp::detail::run_task<Result>(
            node_id, task, std::get<n>(futures).get()...);
    }
};
...
struct call_with_futures_impl<transwarp::wait_type, true, total, n...>
{
                       ...
                       const Tuple& futures)
    {
        transwarp::detail::wait_for_all(std::get<n>(futures)...);
        get_all(
            std::get<n>(futures)...); // ensures that exceptions are propagated
        return transwarp::detail::run_task<Result>(node_id, task);
    }
...
Result call_with_futures(std::size_t node_id, const Task& task,
                         const Tuple& futures)
{
    constexpr std::size_t n = std::tuple_size<Tuple>::value;
    return transwarp::detail::call_with_futures_impl<
        TaskType, 0 == n, static_cast<int>(n)>::template work<Result>(node_id,
                                                                      task,
                                                                      futures);
}
...
void call_with_each_index(transwarp::detail::indices<>, const Functor&,
                          const Tuple&)
{
}

template <std::size_t i, std::size_t... j, typename Functor, typename Tuple>
void call_with_each_index(transwarp::detail::indices<i, j...>, const Functor& f,
                          const Tuple& t)
{
    ...
    f(*ptr);
    transwarp::detail::call_with_each_index(transwarp::detail::indices<j...>(),
                                            f, t);
}

// Calls the functor with every element in the tuple. Expects the tuple to
// contain
// task pointers only and dereferences each element before passing it into the
// functor
template <typename Functor, typename Tuple>
...
{
    static void work(
        const std::tuple<std::shared_ptr<transwarp::task<ParentResults>>...>&
            source,
        std::tuple<std::shared_future<ParentResults>...>& target)
    {
...
struct assign_futures_impl<-1, ParentResults...>
{
    static void work(
        const std::tuple<std::shared_ptr<transwarp::task<ParentResults>>...>&,
        std::tuple<std::shared_future<ParentResults>...>&)
    {
    }
};
...
template <typename... ParentResults>
std::tuple<std::shared_future<ParentResults>...> get_futures(
    const std::tuple<std::shared_ptr<transwarp::task<ParentResults>>...>& input)
{
    std::tuple<std::shared_future<ParentResults>...> result;
    assign_futures_impl<static_cast<int>(sizeof...(ParentResults)) - 1,
                        ParentResults...>::work(input, result);
    return result;
}
...
{
    explicit set_executor_visitor(
        std::shared_ptr<transwarp::executor> executor) noexcept
...
{
    explicit visit(
        const std::function<void(transwarp::itask&)>& visitor) noexcept
        : visitor_(visitor)
    {
    }

    void operator()(transwarp::itask& task) const { task.visit(visitor_); }
...
struct unvisit
{
    void operator()(transwarp::itask& task) const noexcept { task.unvisit(); }
};
...
{
    static_assert(
        std::is_same<TaskType, transwarp::root_type>::value ||
        ...
        "Invalid task type, must be one of: root, accept, accept_any, consume, "
        "consume_any, wait, wait_any");
};

template <typename Functor, typename... ParentResults>
struct result<transwarp::root_type, Functor, ParentResults...>
{
    static_assert(sizeof...(ParentResults) == 0,
...
struct result<transwarp::accept_type, Functor, ParentResults...>
{
    ...
    using type = decltype(std::declval<Functor>()(
        std::declval<std::shared_future<ParentResults>>()...));
};
...
struct result<transwarp::accept_any_type, Functor, ParentResults...>
{
    ...
    using arg_t = typename std::tuple_element<
        0, std::tuple<ParentResults...>>::type; // using first type as reference
    using type = decltype(
        std::declval<Functor>()(std::declval<std::shared_future<arg_t>>()));
};
...
struct result<transwarp::consume_type, Functor, ParentResults...>
{
    ...
    using type =
        decltype(std::declval<Functor>()(std::declval<ParentResults>()...));
};
...
struct assign_node_if_impl<true>
{
    ...
    void operator()(Functor& functor,
                    std::shared_ptr<transwarp::node> node) const noexcept
    {
...
// Assigns the node to the given functor if the functor is a subclass of
// transwarp::functor
template <typename Functor>
void assign_node_if(Functor& functor,
                    std::shared_ptr<transwarp::node> node) noexcept
{
    transwarp::detail::assign_node_if_impl<
        std::is_base_of<transwarp::functor, Functor>::value>{}(functor,
                                                               std::move(node));
}

} // detail

// Executor for sequential execution. Runs functors sequentially on the same
// thread
...
public:
    ...
    std::string get_name() const override { return "transwarp::sequential"; }
    // Runs the functor on the current thread
    void execute(const std::function<void()>& functor,
                 const std::shared_ptr<transwarp::node>&) override
    {
        functor();
    }
};
...
public:
    ...
    // Returns the name of the executor
    std::string get_name() const override { return "transwarp::parallel"; }
...
template <typename TaskType, typename Functor, typename... ParentResults>
class task_impl : public transwarp::task<typename transwarp::detail::result<
                      TaskType, Functor, ParentResults...>::type>,
                  public std::enable_shared_from_this<
                      task_impl<TaskType, Functor, ParentResults...>>
{
public:
    // The task type
    ...
    using result_type =
        typename transwarp::detail::result<task_type, Functor,
                                           ParentResults...>::type;

    // A task is defined by name, functor, and parent tasks. name is optional,
    // see overload
    // Note: A task must be created using shared_ptr (because of
    // shared_from_this)
    template <typename F>
    // cppcheck-suppress passedByValue
    // cppcheck-suppress uninitMemberVar
    task_impl(std::string name, F&& functor,
              std::shared_ptr<transwarp::task<ParentResults>>... parents)
        : task_impl(true, std::move(name), std::forward<F>(functor),
                    std::move(parents)...)
    {
    }
    ...
    // cppcheck-suppress uninitMemberVar
    explicit task_impl(
        F&& functor, std::shared_ptr<transwarp::task<ParentResults>>... parents)
    ...
    void set_executor(std::shared_ptr<transwarp::executor> executor) override
    {
        ...
        executor_ = std::move(executor);
        transwarp::detail::node_manip::set_executor(
            *node_, std::make_shared<std::string>(executor_->get_name()));
    }

    // Assigns an executor to all tasks which takes precedence over
    // the executor provided in schedule() or schedule_all()
    void set_executor_all(
        std::shared_ptr<transwarp::executor> executor) override
    {
        ensure_task_not_running();
    ...
    // Sets a task priority (defaults to 0). transwarp will not directly use
    // this.
    // This is only useful if something else is using the priority (e.g. a
    // custom executor)
    ...
    // Sets a priority to all tasks (defaults to 0). transwarp will not directly
    // use this.
    // This is only useful if something else is using the priority (e.g. a
    // custom executor)
    ...
    // This is only useful if something else is using this custom data (e.g. a
    // custom executor)
    void set_custom_data(std::shared_ptr<void> custom_data) override
    {
        ...
        if (!custom_data)
        {
            throw transwarp::transwarp_error(
                "Not a valid pointer to custom data");
        }
    ...
    {
        ensure_task_not_running();
        transwarp::detail::set_custom_data_visitor visitor(
            std::move(custom_data));
        visit(visitor);
        unvisit();
    }
    ...
    // Schedules all tasks in the graph for execution using the provided
    // executor.
    // The task-specific executors get precedence if they exist.
    // reset_all denotes whether schedule_all should reset the underlying
    // futures and schedule even if the futures are already present.
    void schedule_all(transwarp::executor& executor,
                      bool reset_all = true) override
    {
        ensure_task_not_running();
    ...
    bool was_scheduled() const noexcept override { return future_.valid(); }
    ...
    // Returns the result of this task. Throws any exceptions that the
    // underlying
    ...
    // the tasks and their interdependencies. Pass the result into
    // transwarp::to_string
...
private:
    template <typename F>
    // cppcheck-suppress passedByValue
    task_impl(bool has_name, std::string name, F&& functor,
              std::shared_ptr<transwarp::task<ParentResults>>... parents)
        ...
        , visited_(false)
    {
        ...
        transwarp::detail::node_manip::set_name(
            *node_, (has_name ? std::make_shared<std::string>(std::move(name))
                              : nullptr));
        transwarp::detail::assign_node_if(functor_, node_);
        transwarp::detail::call_with_each(transwarp::detail::parent_visitor(
                                              node_),
                                          parents_);
    ...
    void schedule_impl(bool reset,
                       transwarp::executor* executor = nullptr) override
    {
        if (!node_->is_canceled() && (reset || !future_.valid()))
        {
            ...
            auto pack_task =
                std::make_shared<std::packaged_task<result_type()>>(
                    std::bind(&task_impl::evaluate, node_->get_id(),
                              std::move(self), std::move(futures)));
    ...
    void schedule_all_impl(bool reset_all,
                           transwarp::executor* executor = nullptr)
    {
    ...
    void visit(const std::function<void(transwarp::itask&)>& visitor) override
    {
        if (!visited_)
        {
            transwarp::detail::call_with_each(transwarp::detail::visit(visitor),
                                              parents_);
            visitor(*this);
            visited_ = true;
        }
    ...
    void unvisit() noexcept override
    {
        ...
        {
            visited_ = false;
            transwarp::detail::call_with_each(transwarp::detail::unvisit(),
                                              parents_);
        }
    }

    // Checks if the task is currently running and throws
    // transwarp::transwarp_error if it is
    void ensure_task_not_running() const
    {
                ...
                std::future_status::ready)
        {
            throw transwarp::transwarp_error("the task is currently running: " +
                                             std::to_string(node_->get_id()));
        }
    }

    // Checks if the task was scheduled and throws transwarp::transwarp_error if
    // it's not
    ...
    // Calls the functor of the given task with the results from the futures.
    // Throws transwarp::task_canceled if the task is canceled.
    // Throws transwarp::task_destroyed in case the task was destroyed
    // prematurely.
    static result_type evaluate(
        std::size_t node_id, const std::weak_ptr<task_impl>& task,
        const std::tuple<std::shared_future<ParentResults>...>& futures)
    {
        return transwarp::detail::call_with_futures<task_type, result_type>(
            node_id, task, futures);
    }
...
public:
    ...
    // A value task is defined by name and result. name is optional, see
    // overload
    template <typename R>
    // cppcheck-suppress passedByValue
    // cppcheck-suppress uninitMemberVar
    ...
    // Resets the priority of all tasks to 0
    void reset_priority_all() override { reset_priority(); }
    // Assigns custom data to this task. transwarp will not directly use this.
    ...
    // Removes custom data from all tasks
    void remove_custom_data_all() override { remove_custom_data(); }
    // Returns the future associated to the underlying execution
    const std::shared_future<result_type>& get_future() const noexcept override
    {
        return future_;
    ...
    bool was_scheduled() const noexcept override { return true; }
    // No-op because a value task never runs
    void wait() const override {}
    // Returns true because a value task is always ready
    bool is_ready() const override { return true; }
    // Returns the result of this task
    const result_type& get() const override { return future_.get(); }
    // No-op because a value task never runs
    void reset() override {}
    // No-op because a value task never runs and doesn't have parents
    void reset_all() override {}
    ...
    std::vector<transwarp::edge> get_graph() const override { return {}; }
private:
    template <typename R>
    // cppcheck-suppress passedByValue
...
auto make_task(TaskType, Functor&& functor, std::shared_ptr<Parents>... parents)
    -> decltype(
        std::make_shared<
            transwarp::task_impl<TaskType, typename std::decay<Functor>::type,
                                 typename Parents::result_type...>>(
            std::forward<Functor>(functor), std::move(parents)...))
{
    return std::make_shared<
        transwarp::task_impl<TaskType, typename std::decay<Functor>::type,
                             typename Parents::result_type...>>(
...
auto make_value_task(std::string name, Value&& value) -> decltype(
    std::make_shared<transwarp::value_task<typename std::decay<Value>::type>>(
        std::move(name), std::forward<Value>(value)))
{
    return std::make_shared<
        transwarp::value_task<typename std::decay<Value>::type>>(
        std::move(name), std::forward<Value>(value));
}

// A factory function to create a new value task. Overload for omitting the task
// name
template <typename Value>
auto make_value_task(Value&& value) -> decltype(
    std::make_shared<transwarp::value_task<typename std::decay<Value>::type>>(
