...
public:
    ...
    node(std::size_t id, transwarp::task_type type,
...
struct call_with_futures_impl
{
                       ...
                       const Tuple& futures)
    {
        return call_with_futures_impl<
            TaskType, total == 1 + sizeof...(n), total, n...,
            sizeof...(n)>::template work<Result>(node_id, task, futures);
    }
};
...
struct call_with_futures_impl<transwarp::consume_any_type, true, total, n...>
{
                       ...
                       const Tuple& futures)
    {
        ...
        for (;;)
        {
            bool ready = false;
            auto future =
                waiter<future_t>::template wait(ready, std::get<n>(futures)...);
            if (ready)
            {
                return transwarp::detail::run_task<Result>(node_id, task,
                                                           future.get());
    ...
    struct waiter
    {
        ...
        static Future wait(bool&) { return {}; }
    };
};
                         ...
                         const Tuple& futures)
{
    ...
    return transwarp::detail::call_with_futures_impl<
        TaskType, 0 == n, n>::template work<Result>(node_id, task, futures);
}
...
struct result
{
    static_assert(
        ...
        "Invalid task type, must be one of: root, consume, consume_any, wait, "
        "wait_any");
};
...
private:
    ...
    task_impl(bool has_name, std::string name, F&& functor,
        ...
        : node_(std::make_shared<transwarp::node>(
              0, task_type::value,
              (has_name ? std::make_shared<std::string>(std::move(name))
                        : nullptr),
