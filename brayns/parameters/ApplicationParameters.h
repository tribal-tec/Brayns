...
public:
    ...
    {
        _updateValue(_streamingEnabled, enabled);
    }
    ...
    {
        _updateValue(_streamCompression, enabled);
    }
    ...
    {
        _updateValue(_streamQuality, quality);
    }

    /** Stream ID; defaults to DEFLECT_ID if empty */
    const std::string& getStreamId() const { return _streamId; }
    void setStreamId(const std::string& id) { _updateValue(_streamId, id); }
    ...
    {
        _updateValue(_streamHostname, host);
    }

    /** Stream port; defaults to 1701 if empty */
    uint16_t getStreamPort() const { return _streamPort; }
    void setStreamPort(const uint16_t port) { _updateValue(_streamPort, port); }
