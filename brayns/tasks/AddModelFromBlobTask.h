 ...
 */
class AddModelFromBlobTask : public Task<size_t>
{
public:
...
private:
    ...
    async::event_task<size_t> _errorEvent;
    std::vector<async::task<size_t>> _finishTasks;
    std::string _blob;

    void _checkValidity(EnginePtr engine);
    void _cancel() final
    {
        _chunkEvent.set_exception(std::make_exception_ptr(async::task_canceled()));
    }
