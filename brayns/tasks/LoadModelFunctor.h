...
public:
    ...
    size_t operator()(Blob&& blob);
    size_t operator()(const std::string& path);

private:
    size_t _performLoad(const std::function<size_t()>& loadData);

    size_t _loadData(Blob&& blob);
    size_t _loadData(const std::string& path);
