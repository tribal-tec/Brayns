...
std::string buildJsonRpcSchema(const RpcDescription& desc, P& obj)
{
    using namespace rapidjson;
...
template <class P, class R>
std::string buildJsonRpcSchema(const RpcDescription& desc)
{
    P obj;
    return buildJsonRpcSchema<P, R>(desc, obj);
}
...
{
    P obj;
    return buildJsonRpcSchema<P>(desc, obj);
}
