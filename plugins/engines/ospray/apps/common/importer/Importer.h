...
template <typename T>
inline void extendVoxelRange(ospcommon::vec2f &voxelRange, const T *voxel,
                             size_t num)
{
...
//! Convenient wrapper that will do the template dispatch for you based on the
//! voxelType passed
                             ...
                             const size_t numVoxels)
{
    ...
    default:
        std::cerr << "ERROR: unsupported voxel type "
                  << stringForType(voxelType) << std::endl;
        exit(1);
    }
}
