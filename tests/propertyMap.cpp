...
BOOST_AUTO_TEST_CASE(set_property)
{
    ...
    BOOST_CHECK_EQUAL(properties.getProperties()[0]->userInfo.label, "");
    BOOST_CHECK_EQUAL(properties.getProperties()[0]->userInfo.description,
                      "no-description");
}

BOOST_AUTO_TEST_CASE(set_property_with_userinfo)
{
    brayns::PropertyMap properties;

    properties.setProperty(
        {"foo", 1, brayns::Property::UserInfo{"Foo", "Foo description"}});
    BOOST_REQUIRE_EQUAL(properties.getProperties().size(), 1);
    BOOST_CHECK_EQUAL(properties.getProperties()[0]->get<int32_t>(), 1);
    BOOST_CHECK_EQUAL(properties.getProperties()[0]->userInfo.label, "Foo");
    BOOST_CHECK_EQUAL(properties.getProperties()[0]->userInfo.description,
                      "Foo description");
}
...
BOOST_AUTO_TEST_CASE(set_property_with_limits_and_userinfo)
{
    brayns::PropertyMap properties;
    properties.setProperty(
        {"limit",
         0.5,
         {0., 1.},
         brayns::Property::UserInfo{"With limits", "Limits description"}});
    BOOST_REQUIRE_EQUAL(properties.getProperties().size(), 1);
    BOOST_CHECK_EQUAL(properties.getProperties()[0]->get<double>(), 0.5);
    BOOST_CHECK_EQUAL(properties.getProperties()[0]->min<double>(), 0.);
    BOOST_CHECK_EQUAL(properties.getProperties()[0]->max<double>(), 1.);
